...
\begin{columns}
	...
	\begin{column}{0.48\textwidth}
		\begin{center}
			...
			\node(SelectCluster)[roundrectSmallLong]{Sampling $|\Psi_{CC}\rangle$};
			\node(ConstructCluster)[roundrectSmallLong, below of=SelectCluster, yshift=-0.6cm]{Sampling $\hat{H}$};
			\node(Offdiagonal)[roundrectSmall, below left of=ConstructCluster, yshift=-0.7cm, xshift=-1.1cm ]{Spawn };
			\node(Diagonal)[roundrectSmall, below right of=ConstructCluster, yshift=-0.7cm,xshift=1.1cm]{Death };
			\node(StoreExcip)[roundrectSmallLong, below of=Diagonal, yshift=-0.6cm, xshift=-1.4cm]{Store excips};
			\node(Annihilation)[roundrectSmallLong, below of=StoreExcip, yshift=-0.6cm]{Annihilation};
			\draw [arrow] (SelectCluster) -- (ConstructCluster);
			\draw [arrow] (ConstructCluster) -- (Offdiagonal);
			\draw [arrow] (ConstructCluster) -- (Diagonal);
			\draw [arrow] (Offdiagonal) -- (StoreExcip);
			\draw [arrow] (Diagonal) -- (StoreExcip);	
			\draw [arrow] (StoreExcip) -- (Annihilation);
			\end{tikzpicture}
		\end{center}
	\end{column}
\end{columns}	
