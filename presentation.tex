...
\begin{itemize}
	...
	\end{equation*}}
	\uncover<2->{\item  Express the wavefunction for arbitrary $\tau$ with a propagator:
	\begin{equation*}
...
\begin{frame}{Projector Monte Carlo Methods}
How do we implement Projector Monte Carlo?
\begin{itemize}
	\uncover<1->{
	...
	\uncover<2->{\item  Introduce wave-function for the imaginary time $\tau + \delta \tau$:
		...
		\end{equation*}}
\end{itemize}
\uncover<3->{Now we are ready for the stochastic Coupled Cluster!}
\end{frame}


\begin{frame}
\frametitle{Coupled Cluster Quantum Monte Carlo}
\uncover<1->{Use the exponential ansatz for wave function and project equation on the excited determinant $D_{\{\boldsymbol{i}\}}$:}
